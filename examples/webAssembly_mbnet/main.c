...
unsigned char buffer[128*128*4]={0};
unsigned char img[128*128*3]={0};

tm_mdl_t mdl;

tm_mat_t in_uint8 = {3, IMG_L, IMG_L, 3, {(mtype_t *) img}};
...
EMSCRIPTEN_KEEPALIVE
int predict() {
    //传进来的实际上是rgba数组。去掉alpha通道
    for (int i = 0; i < 128 * 128; ++i) {
         img[i*3+0] =buffer[i*4];
         img[i*3+1] =buffer[i*4+1];
         img[i*3+2] =buffer[i*4+2];
    }
